...
export default async function DashboardPage() {
  ...
  try {
    ...
    const reservationCount = await prisma.tripBooking.count({
      where: currentUser.role === "ADMIN" ? {} : { tripMaster: { listing: { userId: currentUser.id } } }
    });

    // Get tasks (Pending reservations)
    const pendingReservations = await prisma.tripBooking.findMany({
      where: {
        tripMaster: { listing: { userId: currentUser.id } },
        paymentStatus: "PENDING"
      },
      include: { user: true, tripMaster: { include: { listing: true } } },
      take: 5,
                  ...
                  <div key={res.id} className="p-5 bg-gray-50 rounded-2xl flex items-center justify-between group hover:bg-white hover:shadow-md hover:border-gray-200 border border-transparent transition-all duration-300">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 font-medium">{res.user.name} • {res.tripMaster.listing.title}</p>
                      </div>
                    </div>
